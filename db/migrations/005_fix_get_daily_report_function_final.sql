-- Całkowicie przeprojektowana funkcja do pobierania danych raportu dziennego
...
BEGIN
  ...
  WITH start_report AS (
    SELECT 
      ...
      r.report_date,
      r.report_type,
      r.terminal_shipment_report::TEXT,
      r.work_hours,
      r.initial_cash,
      r.deposited_cash,
      r.created_at,
      (u.name || ' ' || u.surname)::TEXT AS user_name
    FROM reports r
  ...
  combined_fruits AS (
    SELECT 
      COALESCE(sf.fruit_name, ef.fruit_name)::TEXT AS fruit_name,
  ...
  SELECT 
    ...
    COALESCE((SELECT sr.terminal_shipment_report FROM start_report sr LIMIT 1), '')::TEXT AS start_terminal_report,
    COALESCE((SELECT er.terminal_shipment_report FROM end_report er LIMIT 1), '')::TEXT AS end_terminal_report,
