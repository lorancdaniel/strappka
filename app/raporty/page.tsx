...
export default function RaportyPage() {
  ...
  useEffect(() => {
    const checkUserRole = async () => {
      try {
        ...
        // Jeśli użytkownik nie jest administratorem, przekieruj na pulpit
  ...
  const fetchReports = async () => {
    try {
      const response = await fetch(
        `/api/reports?placeId=${selectedPlace}&date=${
  ...
  const checkReports = async (placeId: string, date: string) => {
    try {
      ...
      console.log("[FRONTEND] Sprawdzanie raportów dla:", {
        placeId,
        placeIdNumber,
        date,
      });
      ...
      if (data.reports) {
        console.log("[FRONTEND] Znalezione raporty:", {
          ...
          typy: data.reports.map((r: ReportItem) => r.report_type),
        });
      }
        ...
        <div className="flex gap-4 items-end mb-6">
          ...
          <div className="flex-1 space-y-2">
            ...
            <Select value={selectedPlace} onValueChange={setSelectedPlace}>
              ...
              <SelectContent>
                <SelectItem value="1">Miejsce 1</SelectItem>
                <SelectItem value="2">Miejsce 2</SelectItem>
                <SelectItem value="3">Miejsce 3</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
