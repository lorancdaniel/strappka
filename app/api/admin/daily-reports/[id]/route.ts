...
export async function GET(
  ...
  try {
    console.log(
      "[ADMIN_DAILY_REPORT_DETAILS] Rozpoczynam pobieranie szczegółów raportu dziennego"
    );

    // Weryfikacja tokena
    const token = request.cookies.get("token")?.value;
    if (!token) {
      console.log("[ADMIN_DAILY_REPORT_DETAILS] Brak tokena autoryzacji");
      return NextResponse.json({ error: "Brak autoryzacji" }, { status: 401 });
    }

    const payload = await verifyToken(token);
    if (!payload || !payload.id) {
      console.log("[ADMIN_DAILY_REPORT_DETAILS] Nieprawidłowy token");
    ...
    if (payload.role !== "admin") {
      console.log(
        "[ADMIN_DAILY_REPORT_DETAILS] Użytkownik nie jest administratorem"
      );
      return NextResponse.json(
    ...
    if (isNaN(id)) {
      console.log("[ADMIN_DAILY_REPORT_DETAILS] Nieprawidłowe ID raportu");
      return NextResponse.json(
    ...
    console.log("[ADMIN_DAILY_REPORT_DETAILS] ID raportu:", id);
    ...
    if (result.rows.length === 0) {
      console.log("[ADMIN_DAILY_REPORT_DETAILS] Raport dzienny nie istnieje");
      return NextResponse.json(
        { error: "Raport dzienny nie istnieje" },
        { status: 404 }
      );
    }
    ...
    console.log(
      "[ADMIN_DAILY_REPORT_DETAILS] Pobrano szczegóły raportu dziennego"
    );

    return NextResponse.json(report);
  } catch (error) {
    console.error(
      "[ADMIN_DAILY_REPORT_DETAILS] Błąd podczas pobierania szczegółów raportu dziennego:",
      error
    );
    return NextResponse.json(
      {
...
export async function DELETE(
  ...
  try {
    console.log(
      "[ADMIN_DAILY_REPORT_DETAILS] Rozpoczynam usuwanie raportu dziennego"
    );
    ...
    const checkQuery = `
      SELECT id FROM daily_reports WHERE id = $1
    `;
    ...
    console.log(
      "[ADMIN_DAILY_REPORT_DETAILS] Raport dzienny usunięty pomyślnie"
    );

    return NextResponse.json({
      message: "Raport dzienny usunięty pomyślnie",
    });
  } catch (error) {
    console.error(
      "[ADMIN_DAILY_REPORT_DETAILS] Błąd podczas usuwania raportu dziennego:",
      error
    );
    return NextResponse.json(
      { error: "Wystąpił błąd podczas usuwania raportu dziennego" },
      { status: 500 }
    );
  }
}
